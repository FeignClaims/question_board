...
- 条件: 给断点设置命中条件, 可以设置表达式、命中次数等, 当命中条件满足时断点才会被命中. 例如我们只想让 :cpp:`i == 10` 时命中断点, 则可设置条件表达式 `i == 10`.
...
如上图中我在第二次调用 :cpp:`add_1()` 时中断, 则函数调用信息如下所示, 其中行号表示该函数目前执行到的语句:
...
2. 在监视窗口用 :cpp:`reinterpret_cast<int (*)[5]>(array)` 强制转换传入的 :cpp:`int* array`, 恢复数组的长度信息.
...
1. :cpp:`sizeof` 的功能是获取类型的大小. :cpp:`sizeof(array)` 和 :cpp:`sizeof(array[0])` 分别是获取哪个类型的大小?
