...
同样基于范围和迭代器的概念, 标准库在 :cpp:`<algorithm>`、:cpp:`<numeric>` (和 :cpp:`<memory>`) 中定义了 142 种算法 (截止 C++23).
...
当然, 142 种算法不止有查找, 但你已经掌握了算法如何使用, `需要时查询即可 <https://zh.cppreference.com/w/cpp/header/algorithm>`_.
...
解引用
  .. code-block:: cpp
    ...
    T const& operator*() const {
      return value_;  // 解引用意味着使用当前输入
    }

相等比较
  对于此处设计的输入迭代器, 相等比较的唯一目的是判断 :cpp:`begin == end` 是否成立, 这是通过判断 :cpp:`cin` 是否读取失败而不能继续读取, 即 :cpp:`cin.fail()` 是否为 :cpp:`true` 来获得的.
...
构造函数
  我们选择解引用时直接返回 :cpp:`value_`, 这意味着以下代码存在问题:
...
当输入迭代器进行自增操作后, 之前拷贝它所得到的新迭代器出现奇怪的行为, 这是与 :cpp:`cin` 和 :cpp:`cout` 的特点相符的:
...
为完成反向, 范围 :cpp:`[begin, end)` 对应于新的范围 :cpp:`[rbegin, rend)`, 则:
...
.. code-block:: cpp
  ...
  int main() {
    ...
    int index = find(array + 1, 3, 3);
    if (index != 3) {
      // 假设 index == 0, 它对应的是 array[1]!😩
    }
  }
...
现在假设我们要设计一个算法 :cpp:`find_last`, 对于一个必然有满足条件的元素的范围, 它找到最右边一个满足条件的元素, 则根据这些迭代器的差异, 我们可以编写不同效率的算法:
...
.. tabs::
  ...
  .. tab:: 输入流迭代器
    ...
    这不可能做到. 输入流迭代器只能向右遍历, 所以当找到满足条件的元素时, 我们不能确定它就是最右边的元素, 必须继续向右遍历.
    这意味着之前满足条件的元素的迭代器出现奇怪行为, 我们不敢保证正确性.
...
前向迭代器 (forward iterator)
  ...
  - 它只保证多次遍历的有效性: 迭代器发生自增操作不影响之前的迭代器的拷贝.
