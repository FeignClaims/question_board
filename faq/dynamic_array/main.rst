...
你是否遇到过这种情况: 你需要给 :cpp:`object` 赋一个新值 :cpp:`new_value`, 但之后又需要用 :cpp:`object` 的旧值. 你于是写:
...
而 :cpp:`exchange(object, new_value)` 将会非常方便, 它将 :cpp:`new_value` 赋给 :cpp:`object`, 并返回 :cpp:`object` 的旧值:
...
.. code-block:: cpp
  ...
  int* array = new int[5]{};
  use(std::exchange(array, new int[10]{}));
  use(array);
...
发现了吗? 对于有的情况 (尤其是当我们在函数内构造了动态数组, 且想要返回这个数组时), 我们只想拷贝指针, 从而复用这个数组.
